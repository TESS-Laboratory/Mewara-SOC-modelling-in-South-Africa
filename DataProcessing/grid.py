...
class grid:
    ...
    def save_soc_hex_grid():
        ...
        joined_full = grid_utils.get_hex_grid_avg_carbon_color(hex_grid=hex_grid,
                                                          carbon_mapping=carbon_mapping,
                                                          soil_data=soil_data,
                                                          join_type='right') 
        joined_full.to_csv(r'DataProcessing/hex_grid_avg_c.csv')        
        grid_utils.plot_heat_map(soil_data_with_avg_c_color=joined, 
                                 title=f'Long Term Average Carbon (% by Mass) of South Africa (1986-2022)',
                                 savePlot=True,
                                 output_plot_path='Maps/HeatMap.png')
        #input('Press any key to continue')
