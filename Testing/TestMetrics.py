...
class TestMetrics:
    ...
    def predict(self, soc_grid_path, year, lat_lon_pairs, patch_size_meters_landsat, patch_size_meters_climate, patch_size_meters_terrain, save=False):
        ...
        for idx in range(len(lat_lon_pairs)):
            ...
            landsat_patch = training_data_utils.get_landsat_patch(year=year, lat=lat, lon=lon, patch_size_meters=patch_size_meters_landsat)
            terrain_patch = training_data_utils.get_terrain_patch(year=year, lat=lat, lon=lon, patch_size_meters=patch_size_meters_terrain)
            for month in range(1, 13):
                ...
                if soc is not None & soc.any():
                    target_c = soc['C']
                climate_patch = training_data_utils.get_climate_patch(year=year, month=month, lat=lat, lon=lon, patch_size_meters=patch_size_meters_climate) 
        ...
        if save:
            ...
            training_data_utils.save_csv(arr = results, column_names= result_columns, output_path=f'{output_path}\predictions.csv')
