...
lat_lon_pairs = list(set(zip([-29.357073754704118, -28.75085597205501], [31.258000000000003, 29.858])))
...
def test_terrain_patch():
    dem_path =f'Data\TerrainData\Elevation2\DEM_resampled.tif'
    aspect_path = f'Data\TerrainData\Elevation2\Aspect.tif'
    slope_path = f'Data\TerrainData\Elevation2\Slope.tif'
    twi_path = f'Data\TerrainData\Elevation2\TWI.tif'
    ...
    with rasterio.open(dem_path) as dem_dataset, \
        ...
        for lat, lon in lat_lon_pairs:
                ...
                if dem_patch is None or len(dem_patch) == 0:
                    raise ValueError(f"DEM patch for {lat}, {lon} missing")
                ...
                if slope_patch is None or len(slope_patch) == 0:
                    raise ValueError(f"Slope patch for {lat}, {lon} missing")
                ...
                if aspect_patch is None or len(aspect_patch) == 0:
                    raise ValueError(f"Aspect patch for {lat}, {lon} missing")
                ...
                if twi_patch is None or len(twi_patch) == 0:
                    raise ValueError(f"TWI patch for {lat}, {lon} missing")
...
def test_climate_patch():
    ...
    output_prec_folder=f'TestUnits/Patches/{patch_size_meters}/Climate/Prec'
    output_tmin_folder=f'TestUnits/Patches/{patch_size_meters}/Climate/Tmin'
    output_tmax_folder=f'TestUnits/Patches/{patch_size_meters}/Climate/Tmax'
