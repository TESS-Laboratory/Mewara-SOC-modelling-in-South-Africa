...
class data_utils:
    ...
    def replace_nan_data(data):
        mean_val = np.nanmean(data, axis=(0, 1))
        mask = np.isnan(data)
        data[mask] = np.take(mean_val, mask.nonzero()[3])
    ...
    def get_training_data(soc_data_path, start_year, end_year, patch_size):
        ...
        for year in range(start_year, end_year + 1):
            for month in range(1, 13):
                ...
                #data_utils.replace_nan_data(climate_data_monthly)
    ...
    def get_test_data(lat_lon_pairs, start_year, end_year, patch_size):
        landsat_data = []
        climate_data = []
        terrain_data = []
        ...
        for year in range(start_year, end_year + 1):
            for month in range(1, 13):
                climate_data_monthly = data_utils.get_monthly_climate_data(year=year, month=month, lat_lon_pairs=lat_lon_pairs, patch_size=patch_size)
                landsat_data_monthly = data_utils.get_landsat_data(year=year, lat_lon_pairs=lat_lon_pairs, patch_size=patch_size)
                terrain_data_monthly = data_utils.get_terrain_data(lat_lon_pairs=lat_lon_pairs, patch_size=patch_size)
                #data_utils.replace_nan_data(climate_data_monthly)
        ...
        return np.array(landsat_data), np.array(climate_data), np.array(terrain_data)
