...
class base_model:
    ...
    def train_spatial_kfold(model, model_output_path, lat_lon_data, landsat_data, climate_data, terrain_data, targets, epochs, k_fold):
        folds = base_data_utils.spatial_kfold_split(
                                                    ...
                                                    k_fold=k_fold)
        result_test_r2 = []
        for i, (train_data, val_data, test_data) in enumerate(folds):
            print(f"\nTraining {model.__class__.__name__} Fold {i}\n")
            ...
            lat_lon_train_df.to_csv(f'Model/Splits/train_{i}.csv', index=False)

            lat_lon_val_df = pd.DataFrame(lat_lon_val, columns=['Lat', 'Lon'])
            lat_lon_val_df.to_csv(f'Model/Splits/val_{i}.csv', index=False)

            lat_lon_test_df = pd.DataFrame(lat_lon_test, columns=['Lat', 'Lon'])
            lat_lon_test_df.to_csv(f'Model/Splits/test_{i}.csv', index=False)

            print(f"\n Training with {len(lat_lon_train)} train, {len(lat_lon_val)} val and {len(lat_lon_test)} test data")

            test_r2 = model.train(landsat_train = landsat_train, 
